...
namespace Disruptor
{
        ...
        where T : class
    {
        ...
        /// <returns>a constructed ring buffer.</returns>
        /// <exception cref="ArgumentException">if bufferSize is less than 1 or not a power of 2</exception>
        public static RingBuffer<T> CreateSingleProducer(int bufferSize)
        {
            Func<T> factory = () => default(T);
            return CreateSingleProducer(factory, bufferSize, SequencerFactory.DefaultWaitStrategy());
        }
        ...
        {
            long sequence = _sequencerDispatcher.Next();
            try
            {
                var entries = (T[])_entries;
                entries[_bufferPadRef + (int)(sequence & _indexMask)] = item;
