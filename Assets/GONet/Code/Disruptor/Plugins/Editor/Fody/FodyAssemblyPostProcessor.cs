...
public static class FodyAssemblyPostProcessor
{
    ...
    public static void OnPostprocessBuild(BuildTarget target, string pathToBuiltProject)
    {
        ...
        TryProcessAssembliesIn(assemblyPaths, assemblyResolver);
    }
    ...
    static FodyAssemblyPostProcessor()
    {
        ...
        if (TryDoProcessing())
        {
            Debug.Log("Processed at least one assembly, which means IL was modified.  About to request script reload.");
    ...
    {
        Debug.Log("Unity assembly compilation completed.  About to attempt processing.  Assembly: " + arg1);
        if (TryDoProcessing(arg1))
        {
            Debug.Log("[DREETS] Processed at least one assembly, which means IL was modified.  About to request script reload.");
            //EditorUtility.RequestScriptReload();
            //AssetDatabase.Refresh();
        }
    }

    static bool TryDoProcessing(string onlyAllowAssemblyPath = null)
    {
        try
        {
            ...
            foreach( System.Reflection.Assembly assembly in AppDomain.CurrentDomain.GetAssemblies() )
            {
                ...
                try
                {
                    ...
                    {
                        if (onlyAllowAssemblyPath == null || 
                            Path.GetFileName(assembly.Location) == Path.GetFileName(onlyAllowAssemblyPath))
                        {
                            Debug.Log( "Fody processor running.  Allowing assembly to be included: " + assembly.Location);
            ...
            return TryProcessAssembliesIn(assemblyPaths, assemblyResolver);
        }
        catch (Exception e)
        {
            Debug.LogError(e);
    ...
    static bool TryProcessAssembliesIn(HashSet<string> assemblyPaths, IAssemblyResolver assemblyResolver)
    {
        bool didProcessAtLeastOneAssembly = false;

        // Create reader parameters with resolver
        var readerParameters = new ReaderParameters()
        { // SHAUN from  https://github.com/aiczk/Unity-Cecil-Example/blob/master/Assets/Mono%20Cecil%20Sample/Script/CecilTest.cs
            ReadWrite = true,
            InMemory = true,
        ...
        foreach (String assemblyPath in assemblyPaths)
        {
            ...
            */
            ...
            try
            {
                Debug.Log("\tProcessing " + assemblyPath + " ...");

                // Read assembly
                // ORIGINAL: var module = ModuleDefinition.ReadModule(assemblyPath, readerParameters);
                var module = AssemblyDefinition.ReadAssembly(assemblyPath, readerParameters);
                var mainModuleDefinition = module.MainModule;


                PrepareWeaversForModule(weavers, mainModuleDefinition);

                // Process it if it hasn't already
                if (ProcessAssembly(assemblyPath, mainModuleDefinition, weavers))
                {
                    didProcessAtLeastOneAssembly = true;
                    //Stream fileStream = new FileStream(assemblyPath, FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.Read);


                    // ORIGINAL: module.Write(assemblyPath, writerParameters);
                    mainModuleDefinition.Write(assemblyPath);
                    ...
                    //public void Write(string fileName, WriterParameters parameters)
                    {
                        ...
                        //new FileStream(assemblyPath, FileMode.Create, FileAccess.ReadWrite, FileShare.Read);
                        //ModuleWriter.WriteModule(module, Mono.Disposable.Owned(fileStream), writerParameters);
                    }

                    Debug.Log("[DREETS] Done writing for: " + assemblyPath);
                }
            }
            catch (Exception e)
            {
            ...
            {
                // TODO FIXME how to unload from memory to avoid out of memory exception??? module.Dispose();
            }

        }

        return didProcessAtLeastOneAssembly;
    }
    ...
    private static bool ProcessAssembly(string assemblyPath, ModuleDefinition module, IEnumerable<WeaverEntry> weavers)
    {
        ...
        {
            Debug.LogFormat("[DREETS] skipped {0} as it is already processed", assemblyPath);
            return false;
        }

        Debug.Log("Writing to " + assemblyPath + ".  Will execute weaver count: " + weavers.Count());
        ...
        foreach (var weaver in weavers)
        {
            ...
            try
            {
                if (weaver.WeaverInstance is ModuleWeaver moduleWeaver)
                {
                    moduleWeaver.LogDebug = (string msg) => Debug.Log(msg);
                    moduleWeaver.LogWarning = (string msg) => Debug.LogWarning(msg);
                    moduleWeaver.LogError = (string msg) => Debug.LogError(msg);
                }
                weaver.Run("Execute");
                Debug.LogFormat("weaver Execute finished.  weaver.TypeName: {0}, weaver.WeaverInstance.GetType().FullName: {1}, moduleWeaver is ModuleWeaver? {2}", weaver.TypeName, weaver.WeaverInstance.GetType().FullName, weaver.WeaverInstance is ModuleWeaver);
            }
            catch (Exception e)
            {
