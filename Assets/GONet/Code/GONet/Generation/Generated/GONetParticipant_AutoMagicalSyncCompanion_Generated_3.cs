	...
	internal sealed class GONetParticipant_AutoMagicalSyncCompanion_Generated_3 : GONetParticipant_AutoMagicalSyncCompanion_Generated
    {
        ...
        internal override void SerializeSingle(Utils.BitByBitByteArrayBuilder bitStream_appendTo, byte singleIndex)
        {
			switch (singleIndex)
			{
				...
				case 7:
				{ // CircularMotion.NettyWorkedFloat
					// Per-value velocity: Write velocity bit (0 = VALUE, 1 = VELOCITY)
					bitStream_appendTo.WriteBit(nextValueIsVelocity[7]);
				
					if (nextValueIsVelocity[7])
					{
				...
				#if GONET_VELOCITY_SYNC_DEBUG
						GONetLog.Debug($"[VelocitySync][{gonetParticipant.GONetId}] VELOCITY packet[7]: value={currentValue}, velocity={velocity}");
				#endif
				...
				{ // Transform.rotation
					    IGONetAutoMagicalSync_CustomSerializer customSerializer = cachedCustomSerializers[8];
					// Per-value velocity: Write velocity bit (0 = VALUE, 1 = VELOCITY)
					bitStream_appendTo.WriteBit(nextValueIsVelocity[8]);
				
					if (nextValueIsVelocity[8])
					{
						// VELOCITY packet: Calculate and serialize velocity
						var currentValue = Transform.rotation;
						var velocity = CalculateVelocity(8, currentValue, GONetMain.Time.ElapsedTicks);
				#if GONET_VELOCITY_SYNC_DEBUG
						GONetLog.Debug($"[VelocitySync][{gonetParticipant.GONetId}] VELOCITY packet[8]: value={currentValue}, velocity={velocity}");
				#endif
						// Angular velocity (omega) serialization for Quaternion
						// velocity is Vector3 (axis * radians/sec), serialize using Vector3Serializer
						var vector3Serializer = new GONet.Vector3Serializer();
						// TODO: Use velocity quantization settings instead of position settings
						vector3Serializer.Serialize(bitStream_appendTo, gonetParticipant, velocity.UnityEngine_Vector3);
					}
					else
					{
						// VALUE packet: Serialize position normally
				...
				{ // Transform.position
					    IGONetAutoMagicalSync_CustomSerializer customSerializer = cachedCustomSerializers[9];
					// Per-value velocity: Write velocity bit (0 = VALUE, 1 = VELOCITY)
					bitStream_appendTo.WriteBit(nextValueIsVelocity[9]);
				
					if (nextValueIsVelocity[9])
					{
						// VELOCITY packet: Calculate and serialize velocity
						var currentValue = Transform.position;
						var velocity = CalculateVelocity(9, currentValue, GONetMain.Time.ElapsedTicks);
				#if GONET_VELOCITY_SYNC_DEBUG
						GONetLog.Debug($"[VelocitySync][{gonetParticipant.GONetId}] VELOCITY packet[9]: value={currentValue}, velocity={velocity}");
				#endif
						// Serialize velocity (TODO: use velocity quantization settings)
						customSerializer.Serialize(bitStream_appendTo, gonetParticipant, velocity);
					}
					else
					{
						// VALUE packet: Serialize position normally
