...
namespace GONet.Generation
{
	internal sealed class GONetParticipant_AutoMagicalSyncCompanion_Generated_3 : GONetParticipant_AutoMagicalSyncCompanion_Generated
    {
        ...
        internal GONetParticipant_AutoMagicalSyncCompanion_Generated_3(GONetParticipant gonetParticipant) : base(gonetParticipant)
		{
			...
			support7.syncAttribute_VelocityQuantizeLowerBound = -20.000000f;
			support7.syncAttribute_VelocityQuantizeUpperBound = 20.000000f;
			...
			support8.syncAttribute_VelocityQuantizeLowerBound = -0.333369f;
			support8.syncAttribute_VelocityQuantizeUpperBound = 0.333369f;
			support8.velocityQuantizeLowerBound_PerSyncInterval = support8.syncAttribute_VelocityQuantizeLowerBound * UnityEngine.Time.fixedDeltaTime * 1f;
			support8.velocityQuantizeUpperBound_PerSyncInterval = support8.syncAttribute_VelocityQuantizeUpperBound * UnityEngine.Time.fixedDeltaTime * 1f;
			...
			cachedVelocitySerializers[8] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(9, -0.333369f, 0.333369f);
			...
			support9.syncAttribute_VelocityQuantizeLowerBound = -0.019035f;
			support9.syncAttribute_VelocityQuantizeUpperBound = 0.019035f;
			support9.velocityQuantizeLowerBound_PerSyncInterval = support9.syncAttribute_VelocityQuantizeLowerBound * UnityEngine.Time.fixedDeltaTime * 3f;
			support9.velocityQuantizeUpperBound_PerSyncInterval = support9.syncAttribute_VelocityQuantizeUpperBound * UnityEngine.Time.fixedDeltaTime * 3f;
			...
			cachedVelocitySerializers[9] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(18, -0.019035f, 0.019035f);
				...
				{ // CircularMotion.NettyWorkedFloat
					if (isVelocityBundle)
					{
						...
						if (current.GONetSyncType == previous.GONetSyncType && current.GONetSyncType != GONet.GONetSyncableValueTypes.System_Boolean)
						{
							...
							float currentValue = current.System_Single;
							float previousValue = previous.System_Single;
							velocityValue = new GONetSyncableValue();
							velocityValue.System_Single = currentValue - previousValue;
						}
						else
						{
							...
							velocityValue = new GONetSyncableValue();
							velocityValue.System_Single = 0f;
						}
				...
				{ // Transform.rotation
					...
					if (isVelocityBundle)
					{
						...
						if (current.GONetSyncType == previous.GONetSyncType && current.GONetSyncType != GONet.GONetSyncableValueTypes.System_Boolean)
						{
							...
							GONet.GONetLog.Debug($"[AngularVelCalc][{gonetParticipant.GONetId}][idx:8] current={currentValue.eulerAngles}, previous={previousValue.eulerAngles}");
				...
				{ // Transform.position
					...
					if (isVelocityBundle)
					{
						...
						if (current.GONetSyncType == previous.GONetSyncType && current.GONetSyncType != GONet.GONetSyncableValueTypes.System_Boolean)
						{
							...
							GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:9] current={currentValue}, previous={previousValue}");
						
							velocityValue = new GONetSyncableValue();
							velocityValue.UnityEngine_Vector3 = currentValue - previousValue;
						
							GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:9] calculated raw delta={velocityValue.UnityEngine_Vector3}");
						}
						else
						{
							// Type mismatch, use zero delta
							GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:9] Type mismatch or not initialized");
							velocityValue = new GONetSyncableValue();
							velocityValue.UnityEngine_Vector3 = UnityEngine.Vector3.zero;
						}
        ...
        internal override GONet.GONetSyncableValue DeserializeInitSingle_ReadOnlyNotApply(Utils.BitByBitByteArrayBuilder bitStream_readFrom, byte singleIndex, bool useVelocitySerializer = false)
        {
			switch (singleIndex)
			{
				...
				{ // Transform.position
					...
					if (!useVelocitySerializer)
					{
						typedValue += valuesChangesSupport[9].baselineValue_current.UnityEngine_Vector3;
					}
					value.UnityEngine_Vector3 = typedValue;
					return value;
				}
