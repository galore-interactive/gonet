        ...
        internal override void SerializeSingle(Utils.BitByBitByteArrayBuilder bitStream_appendTo, byte singleIndex)
        {
			switch (singleIndex)
			{
				...
				{ // Transform.rotation
					...
					bitStream_appendTo.WriteBit(nextValueIsVelocity[5]);
				
					if (nextValueIsVelocity[5])
					{
						// VELOCITY packet: Calculate and serialize velocity
						var currentValue = Transform.rotation;
						var velocity = CalculateVelocity(5, currentValue, GONetMain.Time.ElapsedTicks);
				#if GONET_VELOCITY_SYNC_DEBUG
						GONetLog.Debug($"[VelocitySync][{gonetParticipant.GONetId}] VELOCITY packet[5]: value={currentValue}, velocity={velocity}");
				#endif
				...
				{ // Transform.position
					    IGONetAutoMagicalSync_CustomSerializer customSerializer = cachedCustomSerializers[6];
					// Per-value velocity: Write velocity bit (0 = VALUE, 1 = VELOCITY)
					bitStream_appendTo.WriteBit(nextValueIsVelocity[6]);
				
					if (nextValueIsVelocity[6])
					{
						// VELOCITY packet: Calculate and serialize velocity
						var currentValue = Transform.position;
						var velocity = CalculateVelocity(6, currentValue, GONetMain.Time.ElapsedTicks);
				#if GONET_VELOCITY_SYNC_DEBUG
						GONetLog.Debug($"[VelocitySync][{gonetParticipant.GONetId}] VELOCITY packet[6]: value={currentValue}, velocity={velocity}");
				#endif
