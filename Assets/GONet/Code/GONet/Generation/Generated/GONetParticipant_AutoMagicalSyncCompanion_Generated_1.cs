...
namespace GONet.Generation
{
	internal sealed class GONetParticipant_AutoMagicalSyncCompanion_Generated_1 : GONetParticipant_AutoMagicalSyncCompanion_Generated
    {
        ...
        internal GONetParticipant_AutoMagicalSyncCompanion_Generated_1(GONetParticipant gonetParticipant) : base(gonetParticipant)
		{
			...
			cachedVelocitySerializers[5] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(9, -0.501652f, 0.501652f);
			...
			cachedVelocitySerializers[6] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(6, -0.057106f, 0.057106f);
					...
					if (isVelocityBundle)
					{
						...
						if (recentChangesCount >= 2)
						{
							...
							else
							{
								...
								GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:5] DeltaTime too small: {deltaTime:F6}s");
								velocityValue = new GONetSyncableValue();
							}
						}
						else
						{
							// Not enough snapshots, use zero velocity
								GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:5] Not enough snapshots: recentChangesCount={recentChangesCount}");
							velocityValue = new GONetSyncableValue();
						}
					...
					if (isVelocityBundle)
					{
						...
						if (recentChangesCount >= 2)
						{
							...
							if (deltaTime > 0.0001f)
							{
								...
								GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:6] calculated velocity={velocityValue.UnityEngine_Vector3}");
							}
							else
							{
								// Delta time too small, use zero velocity
								GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:6] DeltaTime too small: {deltaTime:F6}s");
								velocityValue = new GONetSyncableValue();
							}
						}
						else
						{
							// Not enough snapshots, use zero velocity
								GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:6] Not enough snapshots: recentChangesCount={recentChangesCount}");
							velocityValue = new GONetSyncableValue();
						}
