...
namespace GONet
{
    internal static class CircularMotion_ZeroSync_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class CircularMotion_ZeroSync_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            CircularMotion_ZeroSync_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class CircularMotion_ZeroSync_RpcDispatcher : IRpcDispatcher
    {
        ...
        static CircularMotion_ZeroSync_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(CircularMotion_ZeroSync));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(CircularMotion_ZeroSync), new CircularMotion_ZeroSync_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(CircularMotion_ZeroSync), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for CircularMotion_ZeroSync");
            }
        }
