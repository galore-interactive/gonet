...
namespace GONet
{
    internal static class SpawnTestBeacon_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class SpawnTestBeacon_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            SpawnTestBeacon_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class SpawnTestBeacon_RpcDispatcher : IRpcDispatcher
    {
        ...
        static SpawnTestBeacon_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(SpawnTestBeacon));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(SpawnTestBeacon), new SpawnTestBeacon_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(SpawnTestBeacon), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for SpawnTestBeacon");
            }
        }
