...
{
    internal static class Projectile_ZeroSync_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class Projectile_ZeroSync_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            Projectile_ZeroSync_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class Projectile_ZeroSync_RpcDispatcher : IRpcDispatcher
    {
        ...
        static Projectile_ZeroSync_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(Projectile_ZeroSync));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(Projectile_ZeroSync), new Projectile_ZeroSync_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(Projectile_ZeroSync), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for Projectile_ZeroSync");
            }
        }
