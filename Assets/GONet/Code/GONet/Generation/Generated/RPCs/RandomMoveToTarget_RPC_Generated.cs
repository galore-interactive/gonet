...
namespace GONet
{
    internal static class RandomMoveToTarget_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class RandomMoveToTarget_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            RandomMoveToTarget_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class RandomMoveToTarget_RpcDispatcher : IRpcDispatcher
    {
        ...
        static RandomMoveToTarget_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(RandomMoveToTarget));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(RandomMoveToTarget), new RandomMoveToTarget_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(RandomMoveToTarget), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for RandomMoveToTarget");
            }
        }
