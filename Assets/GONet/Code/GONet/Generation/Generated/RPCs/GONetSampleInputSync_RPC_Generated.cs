...
{
    internal static class GONetSampleInputSync_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class GONetSampleInputSync_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            GONetSampleInputSync_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class GONetSampleInputSync_RpcDispatcher : IRpcDispatcher
    {
        ...
        static GONetSampleInputSync_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(GONetSampleInputSync));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(GONetSampleInputSync), new GONetSampleInputSync_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(GONetSampleInputSync), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for GONetSampleInputSync");
            }
        }
