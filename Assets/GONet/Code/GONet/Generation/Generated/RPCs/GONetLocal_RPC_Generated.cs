...
namespace GONet
{
    internal static class GONetLocal_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class GONetLocal_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            GONetLocal_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class GONetLocal_RpcDispatcher : IRpcDispatcher
    {
        ...
        static GONetLocal_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(GONetLocal));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(GONetLocal), new GONetLocal_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(GONetLocal), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for GONetLocal");
            }
        }
