...
{
    internal static class Projectile_RpcIds
    {
        // Base class RPC IDs
        internal const uint GetFullRpcValidationReport_RpcId = 0xBCF6ED9A;
    }

    internal static class Projectile_RpcInitializer
    {
        [UnityEngine.RuntimeInitializeOnLoadMethod(UnityEngine.RuntimeInitializeLoadType.BeforeSceneLoad)]
        private static void Initialize()
        {
            Projectile_RpcDispatcher.EnsureInitialized();
        }
    }

    internal class Projectile_RpcDispatcher : IRpcDispatcher
    {
        ...
        static Projectile_RpcDispatcher()
        {
            if (!isInitialized)
            {
                isInitialized = true;
                GONetMain.EventBus.RegisterRpcIdMapping(0xBCF6ED9A, nameof(GONetParticipantCompanionBehaviour.GetFullRpcValidationReport), typeof(Projectile));
                GONetMain.EventBus.RegisterRpcDispatcher(typeof(Projectile), new Projectile_RpcDispatcher());
                GONetMain.EventBus.RegisterRpcMetadata(typeof(Projectile), Metadata);
                RegisterRpcHandlers();
                GONetLog.Debug("Initialized RPC dispatcher for Projectile");
            }
        }
