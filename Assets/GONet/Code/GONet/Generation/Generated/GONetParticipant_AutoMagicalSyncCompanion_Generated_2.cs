...
namespace GONet.Generation
{
	internal sealed class GONetParticipant_AutoMagicalSyncCompanion_Generated_2 : GONetParticipant_AutoMagicalSyncCompanion_Generated
    {
        ...
        internal GONetParticipant_AutoMagicalSyncCompanion_Generated_2(GONetParticipant gonetParticipant) : base(gonetParticipant)
		{
			...
			cachedVelocitySerializers[7] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(9, -0.501652f, 0.501652f);
			...
			cachedVelocitySerializers[8] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(6, -0.057106f, 0.057106f);
					...
					if (isVelocityBundle)
					{
						...
						if (current.GONetSyncType == previous.GONetSyncType && current.GONetSyncType != GONet.GONetSyncableValueTypes.System_Boolean)
						{
							...
							if (deltaTime > 0.0001f)
							{
								...
								GONet.GONetLog.Debug($"[AngularVelCalc][{gonetParticipant.GONetId}][idx:7] current={currentValue.eulerAngles}, previous={previousValue.eulerAngles}, deltaTime={deltaTime:F4}s");
					...
					if (isVelocityBundle)
					{
						...
						if (current.GONetSyncType == previous.GONetSyncType && current.GONetSyncType != GONet.GONetSyncableValueTypes.System_Boolean)
						{
							...
							if (deltaTime > 0.0001f)
							{
								...
								GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:8] current={currentValue}, previous={previousValue}, deltaTime={deltaTime:F4}s");
						
								velocityValue = new GONetSyncableValue();
								velocityValue.UnityEngine_Vector3 = (currentValue - previousValue) / deltaTime;
						
								GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:8] calculated velocity={velocityValue.UnityEngine_Vector3}");
							}
							else
							{
								// Delta time too small, use zero velocity
