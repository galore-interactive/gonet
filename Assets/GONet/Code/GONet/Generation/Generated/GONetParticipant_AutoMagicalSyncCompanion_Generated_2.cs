...
namespace GONet.Generation
{
	internal sealed class GONetParticipant_AutoMagicalSyncCompanion_Generated_2 : GONetParticipant_AutoMagicalSyncCompanion_Generated
    {
        ...
        internal GONetParticipant_AutoMagicalSyncCompanion_Generated_2(GONetParticipant gonetParticipant) : base(gonetParticipant)
		{
			...
			support6.velocityQuantizeLowerBound_PerSyncInterval = support6.syncAttribute_VelocityQuantizeLowerBound * UnityEngine.Time.fixedDeltaTime * 1f;
			support6.velocityQuantizeUpperBound_PerSyncInterval = support6.syncAttribute_VelocityQuantizeUpperBound * UnityEngine.Time.fixedDeltaTime * 1f;
			...
			support7.syncAttribute_VelocityQuantizeLowerBound = -0.333369f;
			support7.syncAttribute_VelocityQuantizeUpperBound = 0.333369f;
			support7.velocityQuantizeLowerBound_PerSyncInterval = support7.syncAttribute_VelocityQuantizeLowerBound * UnityEngine.Time.fixedDeltaTime * 1f;
			support7.velocityQuantizeUpperBound_PerSyncInterval = support7.syncAttribute_VelocityQuantizeUpperBound * UnityEngine.Time.fixedDeltaTime * 1f;
			...
			cachedVelocitySerializers[7] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(9, -0.333369f, 0.333369f);

			cachedCustomValueBlendings[7] = GONetAutoMagicalSyncAttribute.GetCustomValueBlending<GONet.PluginAPI.GONetValueBlending_Quaternion_HighPerformance>();
			...
			support8.syncAttribute_PhysicsUpdateInterval = 3;
			support8.isVelocityEligible = true;
			support8.syncAttribute_VelocityQuantizeLowerBound = -0.019035f;
			support8.syncAttribute_VelocityQuantizeUpperBound = 0.019035f;
			support8.velocityQuantizeLowerBound_PerSyncInterval = support8.syncAttribute_VelocityQuantizeLowerBound * UnityEngine.Time.fixedDeltaTime * 3f;
			support8.velocityQuantizeUpperBound_PerSyncInterval = support8.syncAttribute_VelocityQuantizeUpperBound * UnityEngine.Time.fixedDeltaTime * 3f;
			...
			cachedValueSerializers[8] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(18, -125f, 125f);
			cachedVelocitySerializers[8] = GONetAutoMagicalSyncAttribute.GetCustomSerializer<GONet.Vector3Serializer>(18, -0.019035f, 0.019035f);

			cachedCustomValueBlendings[8] = GONetAutoMagicalSyncAttribute.GetCustomValueBlending<GONet.PluginAPI.GONetValueBlending_Vector3_HighPerformance>();
				...
				{ // Transform.rotation
					...
					if (isVelocityBundle)
					{
						...
						if (current.GONetSyncType == previous.GONetSyncType && current.GONetSyncType != GONet.GONetSyncableValueTypes.System_Boolean)
						{
							...
							GONet.GONetLog.Debug($"[AngularVelCalc][{gonetParticipant.GONetId}][idx:7] current={currentValue.eulerAngles}, previous={previousValue.eulerAngles}");
							...
							GONet.GONetLog.Debug($"[AngularVelCalc][{gonetParticipant.GONetId}][idx:7] calculated angularVelocity={angularVelocity} rad/s, degrees/s={angularVelocity * UnityEngine.Mathf.Rad2Deg}");
						}
						else
						{
							// Type mismatch, use zero delta
							GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:7] Type mismatch or not initialized");
							velocityValue = new GONetSyncableValue();
				...
				{ // Transform.position
					...
					if (isVelocityBundle)
					{
						...
						if (current.GONetSyncType == previous.GONetSyncType && current.GONetSyncType != GONet.GONetSyncableValueTypes.System_Boolean)
						{
							...
							GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:8] current={currentValue}, previous={previousValue}");
						
							velocityValue = new GONetSyncableValue();
							velocityValue.UnityEngine_Vector3 = currentValue - previousValue;
						
							GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:8] calculated raw delta={velocityValue.UnityEngine_Vector3}");
						}
						else
						{
							// Type mismatch, use zero delta
							GONet.GONetLog.Debug($"[VelocityCalc][{gonetParticipant.GONetId}][idx:8] Type mismatch or not initialized");
							velocityValue = new GONetSyncableValue();
							velocityValue.UnityEngine_Vector3 = UnityEngine.Vector3.zero;
						}
        ...
        private bool IsVelocityEligible(byte singleIndex)
        {
            switch (singleIndex)
            {
                ...
                case 7: return true; // Transform.rotation (PhysicsUpdateInterval=1)
                case 8: return true; // Transform.position (PhysicsUpdateInterval=3)
            ...
            switch (singleIndex)
            {
                ...
                { // Transform.rotation
                    ...
                    if (mostRecentChangesIndex >= 0 && valueChangeSupport.mostRecentChanges_usedSize > 0)
                    {
                        ...
                        if (angle > 1e-6f)
                        {
                            ...
                            GONet.GONetLog.Debug($"[CLIENT-AngularVel][{gonetParticipant.GONetId}][idx:7] previousRot={previousSnapshot.numericValue.UnityEngine_Quaternion.eulerAngles}, synthesized={synthesized.eulerAngles}, deltaTime={deltaTimeSeconds:F4}s");
                ...
                { // Transform.position
                    ...
                    if (mostRecentChangesIndex >= 0 && valueChangeSupport.mostRecentChanges_usedSize > 0)
                    {
                        ...
                        // DIAGNOSTIC: Log synthesis
                        GONet.GONetLog.Debug($"[CLIENT-Vel][{gonetParticipant.GONetId}][idx:8] previousPos={previousSnapshot.numericValue.UnityEngine_Vector3}, velocity={velocity}, synthesized={synthesized}, deltaTime={deltaTimeSeconds:F4}s");
