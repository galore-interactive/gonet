...
namespace GONet
{
    public static class GONetMain
    {
        ...
        {
            // DIAGNOSTIC: Log scene events entering relay path
            if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
            {
                ...
                string sceneName = eventEnvelope.Event is SceneLoadEvent loadEvt ? loadEvt.SceneName :
                                   eventEnvelope.Event is SceneUnloadEvent unloadEvt ? unloadEvt.SceneName : "UNKNOWN";
                GONetLog.Info($"[SCENE-RELAY-ENTRY] {eventType} for '{sceneName}' - IsReliable={eventEnvelope.IsReliable}, IsSourceRemote={eventEnvelope.IsSourceRemote}, TargetClient={eventEnvelope.TargetClientAuthorityId}");
            }
            ...
            {
                // DIAGNOSTIC: Log if scene event is filtered
                if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
                {
                    string eventType = eventEnvelope.Event.GetType().Name;
                    GONetLog.Warning($"[SCENE-RELAY-FILTERED] {eventType} filtered - LocalOnly={eventEnvelope.Event is ILocalOnlyPublish}, SingularRecip={eventEnvelope.IsSingularRecipientOnly}, IsServer={IsServer}, IsClient={IsClient}, IsSourceRemote={eventEnvelope.IsSourceRemote}");
                }
                return;
            }
            ...
            try
            {
                ...
                // DIAGNOSTIC: Log successful serialization for scene events
                if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
                {
                    string eventType = eventEnvelope.Event.GetType().Name;
                    GONetLog.Info($"[SCENE-RELAY-SERIALIZE] {eventType} serialized - BytesUsed={returnBytesUsedCount}");
                }
            }
            catch (Exception e)
            {
            ...
            // DIAGNOSTIC: Log channel selection for scene events
            if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
            {
                string eventType = eventEnvelope.Event.GetType().Name;
                GONetLog.Info($"[SCENE-RELAY-CHANNEL] {eventType} using channel={channelId} (Reliable={eventEnvelope.IsReliable})");
            }
            ...
            if (IsServer && eventEnvelope.IsSourceRemote)
            {
                ...
                // DIAGNOSTIC: Log send for scene events
                if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
                {
                    GONetLog.Info($"[SCENE-RELAY-SEND] {eventEnvelope.Event.GetType().Name} sent via SendBytesToRemoteConnectionsExceptSourceRemote (excludes Authority {eventEnvelope.SourceAuthorityId})");
                }
            }
            else if (IsServer || !eventEnvelope.IsSourceRemote)
            {
                ...
                if (IsClient)
                {
                    ...
                    if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
                    {
                        GONetLog.Info($"[SCENE-RELAY-SEND] {eventEnvelope.Event.GetType().Name} sent from CLIENT to server");
                    }
                }
                else
                {
                    ...
                    if (shouldBroadcast)
                    {
                        ...
                        if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
                        {
                            uint numConnections = _gonetServer?.numConnections ?? 0;
                            GONetLog.Info($"[SCENE-RELAY-SEND] {eventEnvelope.Event.GetType().Name} broadcasted to ALL {numConnections} clients");
                        }
                    }
                    else
                    {
                        ...
                        if (eventEnvelope.Event is SceneLoadEvent || eventEnvelope.Event is SceneUnloadEvent)
                        {
                            GONetLog.Info($"[SCENE-RELAY-SEND] {eventEnvelope.Event.GetType().Name} sent to targeted client Authority {eventEnvelope.TargetClientAuthorityId}");
                        }
                    }
                }
            }
            ...
            if (Application.isPlaying) // now that [ExecuteInEditMode] was added to GONetParticipant for OnDestroy, we have to guard this to only run in play
            {
                ...
                { // auto-magical sync related housekeeping
                    ...
                    for (int i = 0; i < companion.valuesCount; ++i)
                    {
                        ...
                        // DIAGNOSTIC (Oct 2025): Trace reliability assignment from profile to runtime
                        GONetLog.Info($"[GROUPING-CREATE] Field={monitoringSupport.memberName} Reliability={monitoringSupport.syncAttribute_Reliability} Freq={monitoringSupport.syncAttribute_SyncChangesEverySeconds} VelocityEligible={monitoringSupport.isVelocityEligible} GONetId={gonetParticipant.GONetId}");
        ...
        internal sealed class AutoMagicalSyncProcessing_SingleGrouping_SeparateThreadCapable : IDisposable
        {
            ...
            private int SendSyncValueBundlesToRelevantParties_IfAppropriate(List<AutoMagicalSync_ValueMonitoringSupport_ChangedValue> syncValuesForBundles, long relatedElapsedTicks, Type chosenBundleType)
            {
                ...
                if (count > 0)
                {
                    ...
                    // DIAGNOSTIC (Oct 2025): Trace which channel is actually being used for bundle send
                    // ENHANCED: Show first 3 fields with their current and previous values to detect false changes
                    string fieldDetails = "";
                    int sampleCount = System.Math.Min(3, syncValuesForBundles.Count);
                    for (int i = 0; i < sampleCount; i++)
                    {
                        var change = syncValuesForBundles[i];
                        string fieldName = change.memberName;
                        string currentValue = change.lastKnownValue.ToString();
                        string previousValue = change.lastKnownValue_previous.ToString();
                        uint gonetId = change.syncCompanion?.gonetParticipant?.GONetId ?? 0;
                        fieldDetails += $" [{fieldName}@{gonetId}: {previousValue}→{currentValue}]";
                    }
                    GONetLog.Info($"[BUNDLE-SEND] BundleType={chosenBundleType.Name} ChannelId={useThisChannelId} Count={syncValuesForBundles.Count} GroupingReliability={uniqueGrouping.reliability} GroupingFreq={uniqueGrouping.scheduleFrequency}{fieldDetails}");
        ...
        private static int SerializeBody_ChangesBundle(List<AutoMagicalSync_ValueMonitoringSupport_ChangedValue> changes, Utils.BitByBitByteArrayBuilder bitStream_headerAlreadyWritten, ushort filterUsingOwnerAuthorityId, ref int lastIndexUsed, bool isVelocityBundle)
        {
            ...
            // DIAGNOSTIC (Oct 2025): Log how many changes survived filtering
            // This will help us see if ShouldSendChange is actually filtering out the unwanted changes
            if (!IsServer && changesInBundle > 0)
            {
                GONetLog.Debug($"[BUNDLE-FILTERED] changesInBundle={changesInBundle} (after filtering from {countTotal} queued changes) filterUsingOwnerAuthorityId={filterUsingOwnerAuthorityId} MyAuthorityId={MyAuthorityId} isVelocityBundle={isVelocityBundle}");
            }
