...
namespace NetcodeIO.NET
{
	...
	internal struct NetcodePrivateConnectToken
	{
		...
		public bool Read_PreDualStackSupport(byte[] token, byte[] key, ulong protocolID, ulong expiration, ulong sequence)
		{
        ...
        public bool Read(byte[] token, byte[] key, ulong protocolID, ulong expiration, ulong sequence)
        {
            ...
            try
            {
                ...
                if (tokenLen <= 0)
                {
                    //log("Failed to decrypt token", NetcodeLogLevel.Error);
                    return false;
                }
            }
            catch (Exception e)
            {
                //log($"Exception decrypting token: {e.Message}", NetcodeLogLevel.Error);
                BufferPool.ReturnBuffer(tokenBuffer);
                return false;
            }
            ...
            try
            {
                using (var reader = ByteArrayReaderWriter.Get(tokenBuffer))
                {
                    ...
                    if (numServerAddresses == 0 || numServerAddresses > Defines.MAX_SERVER_ADDRESSES)
                    {
                        //log($"Invalid number of server addresses: {numServerAddresses}", NetcodeLogLevel.Error);
                        BufferPool.ReturnBuffer(tokenBuffer);
                        return false;
                    }
            ...
            {
                //log($"Exception reading token: {e.Message}", NetcodeLogLevel.Error);
                BufferPool.ReturnBuffer(tokenBuffer);
                return false;
            }
            finally
