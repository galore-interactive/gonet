...
public static partial class MemoryPackSerializer
{
    ...
    public static byte[] Serialize<T>(
        in T? value, 
        Func<int, byte[]> gonet_byteArraySource,
        out int gonet_returnBytesUsedCount, 
        out bool gonet_doesNeedToReturnToPool,
        MemoryPackSerializerOptions? options = default)
    {
        gonet_doesNeedToReturnToPool = false;
        if (!RuntimeHelpers.IsReferenceOrContainsReferences<T>())
        {
            //REPLACED BY GONET: var array = AllocateUninitializedArray<byte>(Unsafe.SizeOf<T>());
            gonet_returnBytesUsedCount = Unsafe.SizeOf<T>();
            byte[] array = gonet_byteArraySource(gonet_returnBytesUsedCount);
            gonet_doesNeedToReturnToPool = true;
        ...
        else if (typeKind == TypeHelpers.TypeKind.FixedSizeMemoryPackable)
        {
            ...
            gonet_returnBytesUsedCount = writer.WrittenCount;
        ...
        try
        {
            ...
            Serialize(ref writer, value);
            gonet_returnBytesUsedCount = state.BufferWriter.TotalWritten;
