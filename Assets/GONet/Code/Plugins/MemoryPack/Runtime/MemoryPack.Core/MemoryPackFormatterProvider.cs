...
public static partial class MemoryPackFormatterProvider
{
    ...
    static bool TryInvokeRegisterFormatter(Type type)
    {
        if (typeof(IMemoryPackFormatterRegister).IsAssignableFrom(type))
        {
            ...
            if (m == null)
            {
                m = type.GetMethod("MemoryPack.IMemoryPackFormatterRegister.RegisterFormatter_SHAUN", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
                if (m == null)
                {
                    // Roslyn3.11 generator generate public static method
                    m = type.GetMethod("RegisterFormatter_SHAUN", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
                }
